...
public class Producer implements Runnable {
  ...
  public Producer(Properties props, BlockingQueue<List<String>> msgQueue,
    ...
    String startStr = props.getProperty("startTime", "2015-03-01");
    startTime = new DateTime(startStr).withTimeAtStartOfDay();
    String endStr = props.getProperty("endTime", "2015-03-01T00:02:00.000+08:00");
  ...
  public void run() {
    ...
    try {
      ...
      while (dt.isBefore(endTime)) {
        ...
        dt = dt.plusSeconds(rand.nextInt(60));
   ...
   */
  private String getAction(DateTime dt) {
    Map<String, Object> map = new HashMap<String, Object>();
    map.put("id", String.format("ID-%07d", rand.nextInt(100000)));
    map.put("ts", dt.getMillis());
    List<String> list = new ArrayList<>();
    int keySize = rand.nextInt(18) + 3;
    for (int i = 0; i < keySize; i++) {
      list.add(String.format("KEY%s|V%s", rand.nextInt(200000), rand.nextInt(2000000) + 100));
    }
    list.add("agent|" + deviceAgent[rand.nextInt(deviceAgent.length)]);
    list.add("network|" + netWorks[rand.nextInt(netWorks.length)]);
    list.add("version|" + deviceVersion[rand.nextInt(deviceVersion.length)]);
    list.add("screen|" + osScreen[rand.nextInt(osScreen.length)]);
    list.add("isp|" + isps[rand.nextInt(isps.length)]);
    map.put("testmap", list);

    map.put("vol", String.format("%.2f", rand.nextFloat() + rand.nextInt(100)));
    map.put("ele", rand.nextInt(200));
    map.put("nation", nations[rand.nextInt(nations.length)]);
    map.put("brand", deviceBrand[rand.nextInt(deviceBrand.length)]);
    map.put("model", deviceModel[rand.nextInt(deviceModel.length)]);
    map.put("sysversion", sysVersions[rand.nextInt(sysVersions.length)]);
    map.put("action", eventAction[rand.nextInt(eventAction.length)]);
    map.put("label", eventLabel[rand.nextInt(eventLabel.length)]);
  ...
  public static void main(String[] args) {
    ...
    System.out.println(p.getAction(new DateTime()));
